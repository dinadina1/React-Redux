import { createSlice } from "@reduxjs/toolkit";
import { Zoom, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

// Toastify alert function
const notifyToast = (message) => {
  toast.success(`${message}`, {
    position: "top-right",
    autoClose: 2000,
    theme: "colored",
    transition: Zoom,
    pauseOnHover: false,
  });
};

export const productSlice = createSlice({
  name: "products",
  initialState: {
    product: [
      {
        id: 1,
        title: "iPhone 9",
        description: "An apple mobile which is nothing like apple.",
        price: 549,
        discountPercentage: 12.96,
        rating: 4.69,
        stock: 94,
        brand: "Apple",
        category: "smartphones",
        cart: false,
        thumbnail:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS9h5fFpuVuDtR3cHU99XMONer0jwyZfLyR1w&usqp=CAU",
        quantity: 1,
      },
      {
        id: 2,
        title: "iPhone X",
        description:
          "SIM-Free, Model A19211 6.5-inch Super Retina HD display with OLED technology A12 Bionic chip.",
        price: 899,
        discountPercentage: 17.94,
        rating: 4.44,
        stock: 34,
        brand: "Apple",
        category: "smartphones",
        cart: false,
        thumbnail:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQGd92KbLISjVgXmoD1GOtCkdoMRpgA0WzqGCkK4HJbkWek3qyYT5uDRekCcQBpR7iDsEU&usqp=CAU",
        quantity: 1,
      },
      {
        id: 3,
        title: "Samsung Universe 9",
        description:
          "Samsung's new variant which goes beyond Galaxy to the Universe.",
        price: 1249,
        discountPercentage: 15.46,
        rating: 4.09,
        stock: 36,
        brand: "Samsung",
        category: "smartphones",
        cart: false,
        thumbnail:
          "data:image/jpeg;base64,",
        quantity: 1,
      },
      {
        id: 4,
        title: "OPPOF19",
        description: "OPPO F19 is officially announced on April 2021.",
        price: 280,
        discountPercentage: 17.91,
        rating: 4.3,
        stock: 123,
        brand: "OPPO",
        category: "smartphones",
        cart: false,
        thumbnail:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQnFikN3K-dAx0uFtaLhtFVdf4gwN4kJKCBDeOtxIetuBhZpGQRzViQsJsQqDVf3iOu8Yw&usqp=CAU",
        quantity: 1,
      },
      {
        id: 5,
        title: "Huawei P30",
        description:
          "Huawei’s re-badged P30 Pro New Edition was officially unveiled yesterday in Germany and now the device has made its way to the UK.",
        price: 499,
        discountPercentage: 10.58,
        rating: 4.09,
        stock: 32,
        brand: "Huawei",
        category: "smartphones",
        cart: false,
        thumbnail:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQcpQNmtYwMIwrw6tRlAvm6P4WKRjffc-IK-qnI_BlbtUaX42lR0vWS14G-u97p6VnfInQ&usqp=CAU",
        quantity: 1,
      },
    ],
    cartLists: [],
    totalQuantity: 0,
    totalPrice: 0,
    shippingFee: 0,
    cartCount: 0,
  },
  reducers: {
    // Function add item into cart
    addToCart: (state, action) => {
      let data = state.product.filter(
        (product) => product.id == action.payload
      );
      state.cartLists.push(...data);
      state.product = state.product.map((product) =>
        product.id == action.payload ? { ...product, cart: true } : product
      );

      state.totalQuantity = state.cartLists.reduce(
        (acc, item) => acc + item.quantity,
        0
      );
      let totPrice = (state.totalPrice = state.cartLists.reduce(
        (acc, item) => acc + item.price,
        0
      ));

      if (totPrice > 500) {
        state.shippingFee = 40;
      } else {
        state.shippingFee = 0;
      }

      state.cartCount += 1;
      notifyToast("Successfully added into Cart");
    },

    // Function to increase cart item
    increaseItemQuantity: (state, action) => {
      let input = action.payload.split(",");
      let [id, newQuan, newPrice] = [...input];
      let fixed_price = parseInt(newPrice) / parseInt(newQuan);
      let quans = parseInt(newQuan) + 1;
      let prices = parseInt(quans * fixed_price);

      state.cartLists = state.cartLists.map((product) =>
        product.id == id
          ? { ...product, quantity: quans, price: prices }
          : product
      );

      state.totalQuantity = state.cartLists.reduce(
        (acc, item) => acc + item.quantity,
        0
      );
      let totPrice = (state.totalPrice = state.cartLists.reduce(
        (acc, item) => acc + item.price,
        0
      ));

      if (totPrice > 500) {
        state.shippingFee = 40;
      } else {
        state.shippingFee = 0;
      }
    },

    // Function to decrease cart item
    decreaseItemQuantity: (state, action) => {
      let input = action.payload.split(",");
      let [id, newQuan, newPrice] = [...input];
      let fixed_price = parseInt(newPrice) / parseInt(newQuan);
      let quans = parseInt(newQuan) - 1;

      if (quans > 0) {
        let prices = parseInt(quans * fixed_price);
        state.cartLists = state.cartLists.map((product) =>
          product.id == id
            ? { ...product, quantity: quans, price: prices }
            : product
        );

        state.totalQuantity = state.cartLists.reduce(
          (acc, item) => acc + item.quantity,
          0
        );
        let totPrice = (state.totalPrice = state.cartLists.reduce(
          (acc, item) => acc + item.price,
          0
        ));

        if (totPrice > 500) {
          state.shippingFee = 40;
        } else {
          state.shippingFee = 0;
        }
      }
    },

    // Function to remove cart item
    removeFromCart: (state, action) => {
      state.cartLists = state.cartLists.filter(
        (product) => product.id != action.payload
      );
      state.product = state.product.map((product) =>
        product.id == action.payload ? { ...product, cart: false } : product
      );

      state.totalQuantity = state.cartLists.reduce(
        (acc, item) => acc + item.quantity,
        0
      );
      let totPrice = (state.totalPrice = state.cartLists.reduce(
        (acc, item) => acc + item.price,
        0
      ));

      if (totPrice > 500) {
        state.shippingFee = 40;
      } else {
        state.shippingFee = 0;
      }

      state.cartCount -= 1;
      notifyToast("Successfully removed from Cart");
    },
  },
});

export const {
  addToCart,
  increaseItemQuantity,
  decreaseItemQuantity,
  removeFromCart,
} = productSlice.actions;
